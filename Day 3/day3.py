...
def main():
    all_wires = map(lambda str: str.split(','), input.split())

    print('Starting the program with {} wires'.format(len(all_wires)))
    ...
    for wire in all_wires:
        ...
        for instruction in wire:
            ...
            while(amount>0):
                #print("We are currently in ({}, {}), the Instruction is {} and the amount is {}".format(
                #        x, y, instruction, counter
                #    ))
                if direction == "R":
                    x+=1
                elif direction == "L":
                ...
                if ((x, y) in fullGrid):
                    if(fullGrid[(x, y)][0] != wireNumber):
                        intersectionPoints.append((x, y, totalSteps+fullGrid[(x,y)][1]))
                else:
                    fullGrid[(x, y)] = (wireNumber, totalSteps)
                
        wireNumber += 1
    #print("These are all of the intersection points: {}".format(intersectionPoints))

    finalResultPart1 = functools.reduce(lambda a, b : a if manhattanDistance(a) < manhattanDistance(b) else b, intersectionPoints)
    print("This is the point minimum point based on distance {} with a Manhattan Distance of {}".format(finalResultPart1, manhattanDistance(finalResultPart1)))

    finalResultPart2 = functools.reduce(lambda a, b : a if a[2] < b[2] else b, intersectionPoints)
    print("This is the point minimum point based on steps {} with a total step count of {}".format(finalResultPart2, finalResultPart2[2]))
